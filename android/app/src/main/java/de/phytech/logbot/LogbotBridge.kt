...
class LogbotBridge(private val appContext: Context) {
    ...
    @JavascriptInterface
    fun isBiometricEnabled(): Boolean = Credentials.biometricEnabled(appContext)
    ...
    fun setBiometricEnabled(enabled: Boolean): Boolean {
        ...
        Credentials.setBiometricEnabled(appContext, enabled)
        return true
    }

    /** Aktueller versionName der App, fuer einen About-Eintrag in der Server-UI. */
    @JavascriptInterface
    fun getAppVersion(): String = Credentials.appVersionName(appContext)
}
