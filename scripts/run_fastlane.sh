...
grep -qi "No matching provisioning profiles\|Provisioning profile .* doesn't\|doesn't match the bundle identifier\|No profiles for" "$log" \
  && add "Profil und Bundle-ID passen nicht zusammen. Erwartet wird 'match AppStore de.phytech.logbot'."
...
grep -qi "No suitable application records\|Could not find app with bundle" "$log" \
  && add "Apple findet keine App zu dieser Bundle-ID. In App Store Connect muss ein Datensatz mit de.phytech.logbot existieren."
