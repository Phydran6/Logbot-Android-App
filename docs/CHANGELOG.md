...
- `build-debug.yml` heißt jetzt `ci.yml` und läuft bei jedem Push und jedem Pull Request: Unit-Tests, Lint und Debug-APK in einem Gradle-Aufruf, dazu ein unsignierter iOS-Übersetzungslauf. Lint meldet, bricht aber nicht ab (`abortOnError = false`). Jeder Job hat eine Zeitgrenze.
